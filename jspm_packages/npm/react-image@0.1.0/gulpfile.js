...
var EXAMPLE_COPY = ['node_modules/codemirror/lib/codemirror.js', 'node_modules/codemirror/lib/codemirror.css', 'node_modules/codemirror/mode/javascript/javascript.js', 'node_modules/react/dist/JSXTransformer.js'];
var EXAMPLE_LESS = 'app.less';
var EXAMPLE_FILES = ['index.html'];
function doBundle(target, name, dest) {
  return target.bundle().on('error', function(e) {
    gutil.log('Browserify Error', e);
  }).pipe(source(name)).pipe(gulp.dest(dest)).pipe(connect.reload());
}
...
function buildExampleFiles() {
  return gulp.src(EXAMPLE_FILES.map(function(i) {
    return EXAMPLE_SRC_PATH + '/' + i;
  })).pipe(gulp.dest(EXAMPLE_DIST_PATH)).pipe(connect.reload());
}
...
function buildExampleCSS() {
  return gulp.src(EXAMPLE_SRC_PATH + '/' + EXAMPLE_LESS).pipe(less()).pipe(gulp.dest(EXAMPLE_DIST_PATH)).pipe(connect.reload());
}
...
function buildExampleScripts(dev) {
  ...
  return function() {
    ...
    return merge(doBundle(common, 'common.js', dest), doBundle(bundle, 'bundle.js', dest), doBundle(example, 'app.js', dest));
  };
}
;
gulp.task('dev:build:example:copy', function() {
  return gulp.src(EXAMPLE_COPY).pipe(gulp.dest(EXAMPLE_DIST_PATH)).pipe(connect.reload());
});
...
gulp.task('build:examples', ['build:example:files', 'build:example:css', 'build:example:scripts']);
gulp.task('watch:examples', ['dev:build:example:files', 'dev:build:example:css', 'dev:build:example:scripts', 'dev:build:example:copy'], function() {
  gulp.watch(EXAMPLE_FILES.map(function(i) {
    return EXAMPLE_SRC_PATH + '/' + i;
...
gulp.task('build:dist', ['prepare:dist'], function() {
  var standalone = browserify('./' + SRC_PATH + '/' + PACKAGE_FILE, {standalone: COMPONENT_NAME}).transform(reactify).transform(shim);
  DEPENDENCIES.forEach(function(pkg) {
    standalone.exclude(pkg);
  });
  return standalone.bundle().on('error', function(e) {
    gutil.log('Browserify Error', e);
  }).pipe(source(PACKAGE_NAME + '.js')).pipe(gulp.dest(DIST_PATH)).pipe(rename(PACKAGE_NAME + '.min.js')).pipe(streamify(uglify())).pipe(gulp.dest(DIST_PATH));
});
gulp.task('build', ['build:dist', 'build:examples']);
function getBumpTask(type) {
  return function() {
    return gulp.src(['./package.json', './bower.json']).pipe(bump({type: type})).pipe(gulp.dest('./'));
  };
}
