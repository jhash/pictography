...
(function(process) {
  ...
  module.exports = function(file, options) {
    ...
    if (options.everything) {
      ...
      var extensions = ['js', 'jsx'].concat(options.extension).concat(options.x).filter(Boolean).map(function(ext) {
        return ext[0] === '.' ? ext.slice(1) : ext;
      });
      isJSXFile = getExtensionsMatcher(extensions).exec(file);
    }
    var transformVisitors = [].concat(options.harmony || options.es6 ? visitors.getAllVisitors() : visitors.transformVisitors.react);
    if (options.visitors) {
      if (!VISITORS_OPTION_WARNED) {
        VISITORS_OPTION_WARNED = true;
        console.warn('reactify: option "visitors" is deprecated ' + 'and will be removed in future releases');
      }
      [].concat(options.visitors).forEach(function(id) {
        transformVisitors = require(id).visitorList.concat(transformVisitors);
      });
    }
    var transformOptions = {es5: options.target === 'es5'};
